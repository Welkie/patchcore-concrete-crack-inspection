...
Specifically, the core contributions of this study are four-fold:
\begin{itemize}
    \item \textbf{Methodological Paradigm Shift:} We formulate and systematically implement the cold-start defect detection framework for concrete visual inspection. Unlike traditional supervised binary classifiers that require manual labeling of scarce defective images, our approach models nominal surface textures exclusively, learning to detect cracks as spatial anomalies.
    \item \textbf{Cross-Dataset Generalization Bounds:} We evaluate the generalization ability of unsupervised patch-level embeddings across domains. By training on normal Mendeley concrete textures and testing directly on DeepCrack's unseen topological formations, we show that unsupervised models achieve superior transferability compared to traditional classifiers, which degrade heavily under domain shifts.
    \item \textbf{Feature-Space Compression under Rough Geologies:} We study how coreset memory subsampling behaves when applied to noisy, high-entropy geological surfaces (such as bridge decks with shadows and scaling). We demonstrate that coreset compression acts as a spatial feature regularizer, eliminating local texture noise and rendering the model robust to environmental variations.
    \item \textbf{Quantitative Pixel-Level Localization:} We present a rigorous pixel-level quantitative evaluation of crack localization accuracy (Pixel AUROC) against pseudo-masks. This establishes a baseline for unsupervised spatial defect pinning, showing that high-precision local defect maps can be generated without any pixel-level supervision during training.
\end{itemize}
...
\subsection{Comparative Evaluation on Crack Datasets}
...
\subsection{Resource Efficiency and Latency Trade-Offs}
...
PatchCore (ResNet-18, 10\% coreset ratio) achieves virtually the same Image and Pixel AUROC as SPADE, but reduces the memory index to 62,720 points (122.50 MB, a 90\% saving) and speeds up inference by 5x to 44.28 ms/image, demonstrating the optimal balance for edge-device deployment.

\section{Discussion}
\label{sec:discussion}

\subsection{Unsupervised Generalization and Localization Analysis}
...
\subsection{Performance Curves and Latency Behaviors}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{results/plot_unsupervised_latency_comparison.png}
...
To visually inspect the localization quality, we examine the qualitative crack heatmap overlays for the datasets in Fig.~\ref{fig:localization_surface_crack}, Fig.~\ref{fig:localization_sdnet}, and Fig.~\ref{fig:localization_deepcrack}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{results/visual_localization_surface_crack.png}
    \caption{Qualitative Crack Heatmap Overlays on Mendeley Surface Crack Detection.}
    \label{fig:localization_surface_crack}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{results/visual_localization_sdnet2018_decks.png}
    \caption{Qualitative Crack Heatmap Overlays on SDNET2018 (Decks subset).}
    \label{fig:localization_sdnet}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{results/visual_localization_deepcrack_cross-eval.png}
    \caption{Qualitative Crack Heatmap Overlays on DeepCrack (Cross-dataset evaluation).}
    \label{fig:localization_deepcrack}
\end{figure}

As shown in these figures, the generated anomaly heatmaps successfully trace the physical shape and location of the cracks. The heatmaps display strong peak values precisely along the crack trajectories, with zero or minimal background activation on the normal concrete areas. This localization is achieved without any pixel-level supervision or bounding box annotations during training, demonstrating the robustness of cold-start visual inspection.
